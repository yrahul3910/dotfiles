...
- **A comment that introduces a block sits directly on top of it.** The blank line goes above the comment, never between the comment and the code it describes.
- **The only exemption is a short guard**: an `if` with no `else`, or a loop, whose whole body is one simple statement (`if x is None: return`, `if (!ok) throw new Error(...)`, `for (const p of paths) seen.add(p)`). Guards may stack against their neighbours. Anything with two body statements, a nested block, or an `else` is a block and gets padded.
- **Never write more than about eight consecutive statements without a blank line.** If you have, you skipped a seam. Find it: setup, loop, check, result.
- One blank line, not two, inside a body. The blank lines are for the reader's eye, not for stretching a short function.
...
- Code clumps: see [Blank lines are not optional](#blank-lines-are-not-optional). Not a smell to tolerate in existing code; a defect to fix in yours.
...
- **Inline comments explain *why*.** Don't narrate code that already says what it does. Explain the non-obvious: a tricky invariant, a workaround, a reason for an unusual choice. API documentation also explains what callers can rely on; follow the docstring guidance below.
...
When writing or editing Markdown, follow the [Markdown rule in `unslop`](../unslop/SKILL.md#markdown) for source lines, enforced width limits, and structural exceptions.
