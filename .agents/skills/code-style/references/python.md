...
- Prefer the walrus operator instead of two separate lines: `if (match := PATTERN.search(line)) is None:`, not `match = PATTERN.search(line)` followed by `if match is None:`. `no-sloppy` (SLOP017) enforces this for an assignment directly followed by an `if` that tests it.
