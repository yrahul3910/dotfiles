...
function git(...args: string[]): string {
  try {
    ...
    console.error(
      `no-slop-ts: git ${args[0]} failed: ${stderr.toString().trim()}`,
    );
    process.exit(2);
  }
}
...
function usesEffect(): boolean {
  ...
  try {
    ...
    const pkg = JSON.parse(readFileSync(manifest, "utf-8")) as Record<
      string,
...
function runOxlint(paths: string[], effect: boolean): Finding[] {
  const config = join(
    SKILL_DIR,
    effect ? "oxlintrc.effect.json" : "oxlintrc.json",
  );
...
function render(
  ...
  const covered = Array.from(
    { length: Math.max(last - finding.line + 1, 0) },
    (_, i) => finding.line + i,
  );
  const elided = [
    ...covered.slice(0, EXCERPT_EDGE),
    null,
    ...covered.slice(-EXCERPT_EDGE),
  ];
  const numbers = covered.length > MAX_EXCERPT ? elided : covered;

  const width = Math.max(
    String(last || finding.line).length,
    numbers.includes(null) ? 3 : 1,
  );
  ...
  if (lines !== undefined && numbers.length > 0) {
    ...
    if (numbers.length > 1) {
      ...
      const span = Math.max(
        Math.min(finding.span, display.length - finding.column + 1),
        1,
      );
      out.push(
        `${gutter} ${" ".repeat(finding.column - 1)}${tint}${"^".repeat(span)}${reset}`,
      );
    }
  }

  if (finding.help)
    out.push(`${" ".repeat(width)} ${accent}=${reset} help: ${finding.help}`);
  return out.join("\n");
}
...
function main(): number {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--base") base = argv[++i] ?? base;
      ...
      console.log(
        "usage: no-slop-ts [--base REF] [--all] [--strict] [--effect|--no-effect] [PATH...]",
      );
      return 0;
  ...
  if (paths.length > 0) {
    scope = new Map<string, [number, number][]>(
      paths.map((p) => [resolve(p), [WHOLE_FILE]]),
    );
  } else {
  ...
  const blocks = findings.map((finding) =>
    render(finding, readSource(finding.path)?.lines, color),
  );
