...
2. **Custom rules** -- slop patterns ruff can't express (redundant comments, naming slop, silent fallbacks, ...). One module per rule in [src/no_sloppy/rules/](src/no_sloppy/rules/), auto-discovered at runtime. Current rules:
  ...
  - `SLOP012` a multi-line block (`if`/`for`/`while`/`try`/`with`/`match`/`def`/`class`) with no blank line before or after it. Short guards (an `if` or loop without `else` whose header and single simple body statement fit on one line each) are exempt, as are `@overload` stacks, and a docstring never needs a blank line after it. A `return` directly under two or more statements (guards included) also needs a blank line above it, unless it is the fallback of a dispatch chain of guards that return or raise. A comment directly above a block or `return` belongs to it: the finding anchors on the comment, and a gap holding only a comment still counts as missing
  ...
  - `SLOP015` blank lines that separate nothing: directly under a header, directly above `elif`/`else`/`except`/`finally`, between a comment and its code, two in a row inside a function or class, or anywhere in a function or control-flow body of at most three one-line simple statements, except above a `return` that follows two of them (error; module level follows PEP 8)
