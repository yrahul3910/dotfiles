...
On a long function, or one with several `raise` sites, a single summary
line cannot say what callers get back, what the function guarantees, or
how it fails. Write the contract: the summary line, a blank line, then the
result, the invariants, and the failure outcomes. Short functions with an
obvious contract keep their one-liners. Advisory, because the fix needs
judgement about what the contract actually is.
"""
...
def thin_docstrings(
    ...
    A docstring counts as one line when it has exactly one non-blank line. It is
    flagged when the body after it spans at least `MIN_BODY_LINES` lines or holds
    at least `MIN_RAISES` `raise` sites outside nested scopes. Findings anchor on
    the docstring and are warnings.
    """
    findings = []
