...
diagnostics.__integration = function(window, beacon_config)
  ...
  -- Beacon.
  if package.loaded['plugins.fancydiagnostics.beacon'] then
    if not diagnostics.__beacon then
      diagnostics.__beacon = require('plugins.fancydiagnostics.beacon').new(window, beacon_config)
    else
...
diagnostics.hover = function(window)
  ...
  local beacon_config = vim.tbl_extend('force', get_beacon_config('default', {}, true), get_beacon_config(level, {}, true) or {})
